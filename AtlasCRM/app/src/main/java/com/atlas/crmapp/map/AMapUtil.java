...
public class AMapUtil {
	...
	public static void showerror(Context context, int rCode){
		...
		try {
			switch (rCode) {
				...
				default:
					Toast.makeText(context,context.getString(R.string.t26)+rCode , Toast.LENGTH_LONG).show();
					Log.e("查询失败", "查询失败："+rCode);
					break;
			}
		} catch (Exception e) {
			//Toast.makeText(context, e.getMessage(), Toast.LENGTH_LONG).show();
			Log.e(e.getMessage(), "失败："+rCode);
		}
	}
	...
	public static String getFriendlyTime(int second) {
		if (second > 3600) {
			...
			return hour + ContextUtil.getUtil().getContext().getString(R.string.hour) + miniate + ContextUtil.getUtil().getContext().getString(R.string.min);
		}
		if (second >= 60) {
			int miniate = second / 60;
			return miniate + ContextUtil.getUtil().getContext().getString(R.string.min);
		}
		return second + ContextUtil.getUtil().getContext().getString(R.string.second);
	}
...
//					RouteBusLineItem busline = busStep.getBusLines().get(0);
		...
		public static String getBusPathDes(BusPath busPath) {
			...
			return String.valueOf(time + " | " + subDis + ContextUtil.getUtil().getContext().getString(R.string.t27) + walkDis);
		}
