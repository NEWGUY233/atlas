...
public class UserProfileActivity extends BaseActivity implements OnClickListener{
			...
			public void onSuccess(EaseUser user) {
				if (user != null) {
					...
					if(!TextUtils.isEmpty(user.getAvatar())){
						 Glide.with(UserProfileActivity.this).load(user.getAvatar()).apply(new RequestOptions().placeholder(R.drawable.em_default_avatar)).into(headAvatar);
					}else{
	...
	private void updateRemoteNick(final String nickName) {
		dialog = KProgressHUD.create(this)
				...
				.setLabel(getString(R.string.dl_update_nick))
				.setDetailsLabel(getString(R.string.dl_waiting))
				.setCancellable(true)
				.setAnimationSpeed(2)
				.setDimAmount(0.5f)
				.show();
	...
	private void uploadUserAvatar(final byte[] data) {
		dialog = KProgressHUD.create(this)
				.setStyle(KProgressHUD.Style.SPIN_INDETERMINATE)
				.setLabel(getString(R.string.dl_update_photo))
				.setDetailsLabel(getString(R.string.dl_waiting))
				.setCancellable(true)
				.setAnimationSpeed(2)
				.setDimAmount(0.5f)
