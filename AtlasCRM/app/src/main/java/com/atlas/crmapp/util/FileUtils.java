...
public final class FileUtils {
    ...
    public static File getAtlasResorceFile(){
        File file = new File(Environment.getExternalStorageDirectory(), Constants.FileDir.ATLAS_DIR);
        if (!file.exists()) {
            file.mkdir();
        }
        return file;
    ...
    //获取 一个时间戳的新图片文件
    public static File getNewImageFile(String newImageName){
        /**
         * 这里将时间作为不同照片的名称
         */
        String curImageName = null;
        if(StringUtils.isNotEmpty(newImageName)){
            curImageName = newImageName;
        }else {
            curImageName = String.valueOf(System.currentTimeMillis());
        }
        File output = new File(getAtlasResorceFile(), curImageName + ".jpg");
        /**
