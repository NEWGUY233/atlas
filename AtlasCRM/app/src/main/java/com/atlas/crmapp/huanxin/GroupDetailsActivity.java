...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			...
			case REQUEST_CODE_ADD_USER:// 添加群成员
				...
				progressDialog.setLabel(st1);
				progressDialog.show();
				addMembersToGroup(newmembers);
				break;
			case REQUEST_CODE_EXIT: // 退出群
				progressDialog.setLabel(st2);
				progressDialog.show();
				exitGrop();
				break;
			case REQUEST_CODE_EXIT_DELETE: // 解散群
				progressDialog.setLabel(st3);
				progressDialog.show();
				deleteGrop();
				break;
			...
			case REQUEST_CODE_EDIT_GROUPNAME: //修改群名称
				...
				if(!TextUtils.isEmpty(returnData)){
					progressDialog.setDetailsLabel(st5);
					progressDialog.show();
	...
	private void toggleBlockOfflineMsg() {
		...
		progressDialog.setDetailsLabel("processing...");
		progressDialog.show();
//		final ArrayList list = (ArrayList) Arrays.asList(groupId);
	...
	private KProgressHUD createProgressDialog(){
		if (progressDialog == null) {
			progressDialog = KProgressHUD.create(this)
					.setStyle(KProgressHUD.Style.SPIN_INDETERMINATE)
					//.setLabel("Please wait")
	...
	private void toggleBlockGroup() {
		if(switchButton.isSwitchOpen()){
			...
			progressDialog.setLabel(getString(R.string.Is_unblock));
			progressDialog.show();
			new Thread(new Runnable() {
		        public void run() {
		            try {
