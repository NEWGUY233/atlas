...
public class ArcProgress extends View {
    ...
    private String progressCustomText;
    private int progressCustomTextColor;
    private float progressCustomTextSize;
    private boolean progressCustomTextShow;
    ...
    public ArcProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mContext = context;
        default_text_size = DisplayUtil.sp2px(getResources(), 18);
        min_size = (int) DisplayUtil.dp2px(getResources(), 100);
        default_text_size = DisplayUtil.sp2px(getResources(), 40);
        default_suffix_text_size = DisplayUtil.sp2px(getResources(), 15);
        default_suffix_padding = DisplayUtil.dp2px(getResources(), 4);
        default_suffix_text = "%";
        default_bottom_text_size = DisplayUtil.sp2px(getResources(), 10);
        default_stroke_width = DisplayUtil.dp2px(getResources(), 4);
    ...
    protected void initByAttributes(TypedArray attributes) {
        ...
        // 自定义文字
        progressCustomText = attributes.getString(R.styleable.ArcProgress_arc_custom_text);
        // 自定义文字颜色
        progressCustomTextColor = attributes.getColor(R.styleable.ArcProgress_arc_custom_text_color, default_text_color);
        // 自定义文字大小
        progressCustomTextSize = attributes.getDimension(R.styleable.ArcProgress_arc_custom_text_size, default_text_size);
        // 自定义文字是否显示
        progressCustomTextShow = attributes.getBoolean(R.styleable.ArcProgress_arc_custom_text_show, false);
...
//            textPaint.setTypeface(fontFace);
            ...
            if (progressCustomTextShow){
                textPaint.setTextSize(progressCustomTextSize);
                textPaint.setColor(progressCustomTextColor);
                canvas.drawText(progressCustomText, (getWidth() - textPaint.measureText(text)) / 2.0f, getHeight() / 2 + textHeight / 3, textPaint);
                canvas.drawText(text, (getWidth() - textPaint.measureText(text)) / 2.0f, getHeight() / 2 - textHeight - textHeight / 3, textPaint);
            } else {
            ...
            if (!TextUtils.isEmpty(getProgressTextTop())) {
                ...
                canvas.drawText(getProgressTextTop(), (getWidth() - textPaint.measureText(getProgressTextTop())) / 2.0f, textBaseline + textHeight - DisplayUtil.dp2px(getResources(), 20), textPaint);
            }
            if (!TextUtils.isEmpty(getProgressTextBottom())) {
                ...
                canvas.drawText(getProgressTextBottom(), (getWidth() - textPaint.measureText(getProgressTextBottom())) / 2.0f, textBaseline - textHeight / 2 + DisplayUtil.dp2px(getResources(), 5), textPaint);
