...
import com.atlas.crmapp.common.glide.CircleTransform;
import com.atlas.crmapp.common.glide.CornersTransform;
...
public class GlideUtils {
    ...
    //自定义 imageview 加载
    public static void loadCustomImageView(Context mContext, int placeDrawable, String path, final ImageView mImageView){
        try{
            Glide.with(mContext).load(path).apply(new RequestOptions().dontAnimate().placeholder(placeDrawable).centerCrop()).into(mImageView);
        }catch (Exception ex){
            Logger.d(ex.getMessage());
        }
       /* Glide.with(mContext).load(path).asBitmap().placeholder(drawable).into(new SimpleTarget<Bitmap>() {
            @Override
    ...
    //自定义 imageview 加载
    public static void loadCustomImageView(Context mContext, int placeDrawable, File imageFile, final ImageView mImageView){
        Glide.with(Atlas.getAppContext()).load(imageFile).apply(new RequestOptions().dontAnimate().placeholder(placeDrawable).centerCrop()).into(mImageView);

    }

    public static void loadImageCircleView(Context mContext, int placeDrawable, String path, ImageView mImageView){
        Glide.with(mContext).load(path).apply(new RequestOptions().centerCrop().transform(new CircleTransform(mContext)).placeholder(placeDrawable).error(placeDrawable))
//                .crossFade()
                .into(mImageView);
    }

//    public static void loadImageCornersView(Context mContext, int placeDrawable, String path, ImageView mImageView){
//        Glide.with(mContext).load(path).apply(new RequestOptions().transform(new CenterCrop(),new CornersTransform(mContext)).placeholder(placeDrawable).error(placeDrawable))
////                .crossFade()
//                .into(mImageView);
//    }
    ...
    public static void loadImageViewLoding(Context mContext, String path, ImageView mImageView) {
       Glide.with(mContext).load(path).apply(new RequestOptions().placeholder(R.drawable.product_thum).error(R.drawable.product_thum)).into(mImageView);
    }
    public static void loadImageViewLoding(Context mContext, int path, ImageView mImageView) {
        Glide.with(mContext).load(path).apply(new RequestOptions().placeholder(R.drawable.product_thum).error(R.drawable.product_thum)).into(mImageView);
    }
...
//    public static void loadGif(Context context, int drawableId, ImageView imageView){
//        Glide.with(context).load(drawableId).asGif().into(imageView);
//
//    }
